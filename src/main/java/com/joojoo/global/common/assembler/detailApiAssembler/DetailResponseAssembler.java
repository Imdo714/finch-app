package com.joojoo.global.common.assembler.detailApiAssembler;
...
public class DetailResponseAssembler {

    private final BlockDataFetcher blockDataFetcher;

    /** 주어진 날짜 리스트와 블록/트레이드로그 목록을 응답용 DTO를 조립합니다. */
    public DailyBlockDetailsResponse assembleTagsAndTickersDetailResponse(List<LocalDate> targetDates, List<Block> allBlocks, List<TradeLog> allTradeLogs) {
        /** ID 추출 */
    ...
    private DailyBlockDetailsResponse buildBlockTagsResponse(List<LocalDate> targetDates, List<DailyGroupResponseDto> dailyGroups) {
        boolean hasNext = targetDates.size() > 2;
        LocalDate nextDate = hasNext ? targetDates.get(2) : null;
        return DailyBlockDetailsResponse.of(dailyGroups, hasNext, nextDate);
    }
}
