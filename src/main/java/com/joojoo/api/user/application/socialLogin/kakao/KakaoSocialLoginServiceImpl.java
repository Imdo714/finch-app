...
public class KakaoSocialLoginServiceImpl implements KakaoSocialLoginService {
    ...
    public LoginResponse kakaoSocialLogin(String code) {
        ...
        String refreshToken = jwtTokenUseCase.createAndSaveRefreshToken(user.getUserId(), user.getName(), user);
        String accessToken = jwtTokenUseCase.createAccessToken(user.getUserId(), user.getName());
